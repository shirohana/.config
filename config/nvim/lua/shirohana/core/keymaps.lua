...
local Utils = require 'shirohana.utils.save_file'
...
keymap('i', Keys['<M-h>'], '<Esc><Cmd>wincmd h<CR>', { desc = 'Go to window in h' })
keymap('i', Keys['<M-j>'], '<Esc><Cmd>wincmd j<CR>', { desc = 'Go to window in j' })
keymap('i', Keys['<M-k>'], '<Esc><Cmd>wincmd k<CR>', { desc = 'Go to window in k' })
keymap('i', Keys['<M-l>'], '<Esc><Cmd>wincmd l<CR>', { desc = 'Go to window in l' })
keymap('i', Keys['<M-H>'], '<Esc><Cmd>WindowSwapLeft<CR>', { desc = 'Swap window to h' })
keymap('i', Keys['<M-J>'], '<Esc><Cmd>WindowSwapDown<CR>', { desc = 'Swap window to j' })
keymap('i', Keys['<M-K>'], '<Esc><Cmd>WindowSwapUp<CR>', { desc = 'Swap window to k' })
keymap('i', Keys['<M-L>'], '<Esc><Cmd>WindowSwapRight<CR>', { desc = 'Swap window to l' })
keymap('t', Keys['<M-h>'], '<C-Bslash><C-n><Cmd>wincmd h<CR>', { desc = 'Go to window in h' })
keymap('t', Keys['<M-j>'], '<C-Bslash><C-n><Cmd>wincmd j<CR>', { desc = 'Go to window in j' })
keymap('t', Keys['<M-k>'], '<C-Bslash><C-n><Cmd>wincmd k<CR>', { desc = 'Go to window in k' })
keymap('t', Keys['<M-l>'], '<C-Bslash><C-n><Cmd>wincmd l<CR>', { desc = 'Go to window in l' })
keymap('t', Keys['<M-H>'], '<C-Bslash><C-n><Cmd>WindowSwapLeft<CR>', { desc = 'Swap window to h' })
keymap('t', Keys['<M-J>'], '<C-Bslash><C-n><Cmd>WindowSwapDown<CR>', { desc = 'Swap window to j' })
keymap('t', Keys['<M-K>'], '<C-Bslash><C-n><Cmd>WindowSwapUp<CR>', { desc = 'Swap window to k' })
keymap('t', Keys['<M-L>'], '<C-Bslash><C-n><Cmd>WindowSwapRight<CR>', { desc = 'Swap window to l' })
...
keymap('n', 'zx', function()
  ...
  -- 呼叫套件指令前，檢查 'hlslens' 是否已加載
  local ok, hlslens = pcall(require, 'hlslens')
  if ok then
    hlslens.start()
    -- 如果未安裝 hlslens，提示用戶或執行其他操作
    -- vim.notify("hlslens 未安裝或載入失敗", vim.log.levels.WARN)
    -- return
  end
